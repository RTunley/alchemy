...
class OverviewSection(StudentReportSection):
    ...
    def build_self(self):
        ...
        self.student_paper_summary = report_calc.StudentPaperSummary(self.student, self.clazz, self.paper, scores)
...
class AdjacentGradesSection(StudentReportSection):
    ...
    def build_self(self):
        scores = models.Score.query.filter_by(student_id = self.student.id, paper_id = self.paper.id).all()
        self.student_paper_summary = report_calc.StudentPaperSummary(self.student, self.clazz, self.paper, scores)
        self.adjacent_grades = report_calc.AdjacentGrades(self.clazz.course.grade_levels, self.student_paper_summary.percent_total, self.student_paper_summary.grade, self.student_paper_summary.paper_total)
